...
import { ComposedChart, Area, Line, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, Legend, Brush } from 'recharts';
import { SubTab, IndicatorMetrics } from '../../types';
import { WindDataService } from '../../services/windDataService';
import { IndicatorWrapper } from './IndicatorWrapper';

export const YieldGapView: React.FC<{ onHeaderDataUpdate: (data: any) => void }> = ({ onHeaderDataUpdate }) => {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      try {
        const res = await WindDataService.getIndicatorData(SubTab.EB_YIELD_GAP);
        if (res && res.data) {
          ...
          onHeaderDataUpdate({
            title: "BOCIASI 情绪指标 - 股债收益差",
            metrics: res.metrics,
            subtitle: ""
          });
        }
      } catch (err) {
        console.error("Failed to load yield gap", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <IndicatorWrapper
      ...
      loading={loading}
      metrics={metrics}
      footerNote="数据源: BOCIASIV2.xlsx (BA列)"
    >
        ...
        {!loading && latestRow && (
          <div className="mx-4 mt-2 bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
            ...
            <div className="p-4 grid grid-cols-2 md:grid-cols-3 gap-6 bg-gray-50/50">
              ...
              <div className="space-y-1 border-r border-gray-100">
                <p className="text-[10px] font-bold text-gray-400">收益差读数</p>
                <div className="flex items-baseline">
                  <p className="text-xl font-black text-[#a31f35] font-mono">
            ...
            <ComposedChart data={chartData} margin={{ top: 30, right: 30, left: -10, bottom: 40 }}>
              <defs>
                <linearGradient id="yieldGrad" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor="#a31f35" stopOpacity={0.2} />
                  <stop offset="95%" stopColor="#a31f35" stopOpacity={0} />
                </linearGradient>
              </defs>
              <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e2e8f0" />
              <XAxis dataKey="date" height={50} axisLine={true} tickLine={false} tick={{ fontSize: 10, fill: '#64748b' }} minTickGap={100} />

              <YAxis yAxisId="left" orientation="left" axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: '#a31f35', fontWeight: 'bold' }} domain={['auto', 'auto']} tickFormatter={(v) => `${(v * 100).toFixed(2)}%`} />
              ...
              <Tooltip
                ...
                formatter={(value: any, name: string) => {
                  if (name === "收益差") return [`${(value * 100).toFixed(2)}%`, name];
              ...
              <Area yAxisId="left" type="monotone" dataKey="value" name="收益差" stroke="#a31f35" strokeWidth={2} fill="url(#yieldGrad)" connectNulls={true} fillOpacity={1} />
